  ...
  const fetchSliderImages = async () => {
    ...
    try {
      ...
      if (docSnapshot.exists()) {
        ...
        const sortedImages = images.sort((a: SliderImage, b: SliderImage) => {
  ...
  const handleAddImage = async () => {
    ...
    try {
      ...
      if (docSnapshot.exists()) {
        ...
        // Create the new image data (without createdAt for array updates)
        const newImageData: SliderImage = {
          imageUrl: uploadedImageUrl,
          redirectionURL: redirectionUrl,
          position: position,
          // Remove createdAt from the array item
        };
        ...
        const newImageData: SliderImage = {
          ...
          redirectionURL: redirectionUrl,
          position: position,
          createdAt: serverTimestamp(), // This is fine for document creation
        };
        ...
        await setDoc(docRef, {
          images: finalImages,
          createdAt: serverTimestamp(), // Add timestamp at document level
        });
      }
  
      // Success message
      if (wasPositionTaken) {
        message.success(`Image added at position ${position}. Other images have been shifted accordingly.`);
      } else {
        message.success(`Image added successfully to carousel at position ${position}`);
      }
                      ...
                      <tr key={`${item.position}-${index}`} className="border-b border-gray-200 dark:border-white/10 last:border-0">
                        ...
                        <td className="px-4 py-5 text-left">
                          <a
                            ...
                            className="text-blue-600 hover:text-blue-800 underline break-words max-w-xs inline-block"
                          >
                            {item.redirectionURL}
                          </a>
                        </td>
      ...
      <Modal
        ...
        footer={[
          ...
          <Button
            ...
            disabled={!uploadedImageUrl || !redirectionUrl.trim() || !position || fileUploading}
          >
            Add to Carousel
          </Button>
        ]}
          ...
          <div className="mb-8">
            ...
            <div className="text-xs text-gray-500 mt-1">
              {isPositionTaken(position) && position ? (
                <span className="text-amber-600">ℹ️ Position {position} is taken. Existing images will shift down automatically.</span>
              ) : (
